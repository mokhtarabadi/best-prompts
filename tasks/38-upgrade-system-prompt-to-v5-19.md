# Task: Upgrade system prompt to V5.19.0

**File:** `tasks/38-upgrade-system-prompt-to-v5-19.md`
**Type:** improvement
**Status:** closed

## Goal

Upgrade `system-prompt.md` to V5.19.0 with Google-aligned agentic reasoning, strict grounding constraint, and improved OpenCode tool mechanics in the implementation task template.

## Manager's Notes

- Bump `<system_version>` from 5.18.0 to 5.19.0
- Replace `<agentic_reasoning>` block with Google's official 10-step nested reasoning framework
- Add Strict Grounding bullet to `<constraints>`
- Update `<opencode_implementation_task_template>` with OpenCode mechanics (subagent delegation, apply_patch path markers, output truncation gate)

## Local TODOs

- [x] Initial codebase exploration — read system-prompt.md
- [x] Step 1: Create tasks/38-upgrade-system-prompt-to-v5-19.md
- [x] Step 2: Update system_version to 5.19.0
- [x] Step 3: Replace agentic_reasoning block with Google-aligned version
- [x] Step 4: Add Strict Grounding bullet to constraints
- [x] Step 5: Update opencode_implementation_task_template with mechanics
- [x] Step 6: Verify functionality — run prettier
...
| File                                         | Change                                           |
| -------------------------------------------- | ------------------------------------------------ |
| `system-prompt.md`                           | Multiple edits across 4 blocks                   |
| `CHANGELOG.md`                               | Added system prompt upgrade entries under 5.19.0 |
| `tasks/38-upgrade-system-prompt-to-v5-19.md` | Created as tracking task file                    |

### Changes Applied to `system-prompt.md`

1. **`<system_version>`** (line 1): Bumped from `5.18.0` to `5.19.0`.

2. **`<agentic_reasoning>`** (lines 90-103): Replaced entirely with Google's official 10-step nested reasoning framework. Key improvements:
   - Steps restructured with numbered sub-points (1.1, 1.2, etc.) for precision.
   - Rule 1.2.1: Explicitly addresses user requesting actions in random order — agent must reorder for success.
   - Rule 2.1: Added tool-preference — prefer calling tools with available info over asking user.
   - Rule 3.1-3.3: Deeper hypothesis exploration guidance with prioritization.
   - Rule 8.2: Explicit retry logic — retry on transient errors, change strategy on permanent ones.
   - Rule 10 unchanged — visible reasoning log remains mandatory.

3. **`<constraints>`** (lines 223-235): Added **Strict Grounding** bullet — enforces that the assistant relies only on provided context, treating external knowledge as unsupported.

4. **`<opencode_implementation_task_template>`** (lines 144-209):
   - **Context phase**: Updated subagent delegation line to include `@scout` (external docs research) and `@general` (complex multi-step research) alongside existing `@explore`.
   - **Execution phase**: Updated `apply_patch` rule to require path marker syntax relative to project root (e.g., `*** Add File: <path>`) followed by `@@ ... @@` unified diff format.
   - **Bash phase**: Added CRITICAL RULE 3 — instructs OpenCode to pipe massive test output through `grep`/`tail` to avoid the 50KB truncation limit.
...
--- /dev/null
+++ b/docs/gemini-prompting-strategies.md
@@ -0,0 +1,664 @@
...
+
+See the Gemini thinking documentation for more detail.
+
...
+To learn about image prompting, see the Nano Banana and Imagen prompt guides.
+To learn about video prompting, see the Veo prompt guide.
+Was this helpful?
+Send feedback
\ No newline at end of file
diff --git a/system-prompt.md b/system-prompt.md
index b903cbf..1ea8528 100644
--- a/system-prompt.md
+++ b/system-prompt.md
@@ -1,4 +1,4 @@
