...
Create a new directory structure `user-prompts/` to store reusable, copy-paste prompts for the Manager, and implement the first template `user-prompts/session-compactor.md` for context compaction and cold-start restoration.

## Manager's Notes

- Standardize on `user-prompts/` folder name (reusable prompts).
- Create `session-compactor.md` to prevent context bloat (resetting chat session token counts safely).
- Sync with the CHANGELOG.

## Local TODOs

- [x] Create user-prompts/ directory
- [x] Create user-prompts/session-compactor.md containing the Context Restoration prompt template
- [x] Integrate project overview, task registry tables, codebase forensic states, and cold-start instructions in the template
- [x] Update CHANGELOG.md

## OpenCode Execution Log & Reasoning

- Designed the new reusable prompts structure `user-prompts/` at root.
- Implemented `session-compactor.md` containing a highly detailed context-restoration prompt.
- This allows resetting Gemini's 1-million-token context load while maintaining 100% operational fidelity in a fresh chat.
- Updated CHANGELOG.md under [Unreleased] to track this folder addition.
